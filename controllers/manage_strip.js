            ...
            function (err, image) {
                ...
                .exec((err,data)=>{
                   if(err){
                    ...
                    stripe.findByIdAndUpdate({_id: stripeId},{$push:{ manage_stripe : data._id }})
                    .exec((err,data)=>{
                        if(err){
                            res.send({error:'manage stripe is not add in stripe'})
                        }
                        else{
                            res.send({msg:'manage stripe is add in stripe with image'})
                            }
                        })
                   }
                })    
                ...
                .exec((err,data)=>{
                    if(err){
                        ...
                        res.send({msg:'manage stripe is add in stripe'})
                        }
                    })
              }
        }
